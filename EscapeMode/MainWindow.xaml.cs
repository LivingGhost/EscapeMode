...
namespace EscapeMode
{
    ...
    {
        // 外部からMainWindowを操作させるために保持する
        public static MainWindow Window { get; private set; }

        private static readonly MainWindowViewModel mwvm = new MainWindowViewModel();
        ...
        public MainWindow()
        {
            ...
            // DataContextにMainWindowViewModelを設定する
        ...
        ~MainWindow()
        {
            ...
            if (ScreenStateNotify != null)
                UnregisterPowerSettingNotification(ScreenStateNotify);
        }
        ...
        protected override void OnSourceInitialized(EventArgs e)
        {
            ...
            if (hwndSource != null)
            {
                ...
                ScreenStateNotify = RegisterPowerSettingNotification(hwndSource.Handle, ref GUID_LIDSWITCH_STATE_CHANGE, DEVICE_NOTIFY_WINDOW_HANDLE);
            }
        }
        ...
        [DllImport("User32.dll", SetLastError = true, EntryPoint = "LockWorkStation", CallingConvention = CallingConvention.StdCall)]
        ...
        private IntPtr ScreenStateNotify;
        #endregion
        ...
        private static IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            ...
            WindowMessageParameter subCode = (WindowMessageParameter)wParam.ToInt32();

            if (((Item)mwvm.Item).Lock == true)
            {
                if (message == WindowMessage.WM_POWERBROADCAST && subCode == WindowMessageParameter.PBT_POWERSETTINGCHANGE)
                {
                    ...
                    {
                        bool isLidClose = setting.Data == 0;
                        if (isLidClose)
                        {
                            // ノートPC画面を閉じたらロック
                            LockWorkStation();
            ...
            // Alt + F4で閉じない (ただしMainWindowは非表示にする)
            if ((message == WindowMessage.WM_SYSKEYDOWN) && (subCode == WindowMessageParameter.VK_F4))
            {
                handled = true;
            ...
            // タスクバーの右クリックメニュー「閉じる」を選んでも閉じない (ただしMainWindowは非表示にする)
            if ((message == WindowMessage.WM_SYSCOMMAND) && (subCode == WindowMessageParameter.SC_CLOSE))
            {
                handled = true;
